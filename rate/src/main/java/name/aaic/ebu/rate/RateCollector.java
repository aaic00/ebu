...
public class RateCollector {
  ...
  public void collect(final long itemId, final int spuId, final long sellerId, final int total)
    ...
    int count = 0;
    int page = 1;
    final File file = new File(String.format(RateCollector.FILE, itemId));
    ...
    for (page = 1; page <= totalPage; page++) {
      ...
      for (final JsonNode rate : rateList) {
        final String content = rate.get("rateContent").textValue();
        FileUtils.write(file, content, "UTF8", true);
        if (rate.has("appendComment") && rate.get("appendComment").has("content")) {
          final String append = rate.get("appendComment").get("content").textValue();
          FileUtils.write(file, "\n追评：", "UTF8", true);
          FileUtils.write(file, append, "UTF8", true);
        }
        count++;
        if (count >= total) {
          break;
  ...
  private JsonNode get(final long itemId, final int spuId, final long sellerId, final int page)
    ...
    final String url = String.format(RateCollector.URL, itemId, spuId, sellerId, page);
